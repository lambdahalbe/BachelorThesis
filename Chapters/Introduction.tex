\section{Motivation}
The big relevance of electronic transport in organic semiconductors can be seen by looking at the variety of its applications including photovoltaic, OLED's and many more. To understand the underlying phenomena, including a description of the electronic band structure, a simple model of a perfect, infinite, one-dimensional polymer chain is often assumed. Therefore a well-tried example is polyacetylene, which was already used in the 1950s by \textsc{Salem} and \textsc{Longuett-Higgins} and later in the 1980s by \textsc{Su, Schrieffer} and \textsc{Heeger} to describe the electronic structure under periodic boundary conditions or effects of charge . In this connection a tight binding picture describing the charge transport by electron hopping has shown to yield a good description of the band structure with very few parameters (see \cite{koehler2015electronic,Longuet-Higgins172,PhysRevLett.42.1698}).\\
In this thesis the tight binding parameters for \emph{trans}-polyacetylene are derived with density functional theory. Furthermore, the effects of a periodic charging by using constrained density functional theory on the energy and band structure are tested and different approaches to model this effects are tried. Finally, a attempt of calculating the hopping parameter from the energies of the charged states is shown by employing the predictions of our approaches. This method would be interesting to calculate hopping parameter for more complicate systems, since many effects, such as band interactions, can make a direct fit to the band structure very difficult.
